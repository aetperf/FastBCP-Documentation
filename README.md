...
- `{database}` : value passed to `--database`
- `{sourcedatabase}` : value passed to `--sourcedatabase`
- `{schema}` : value passed to `--schema`
- `{sourceschema}` : value passed to `--sourceschema`
- `{table}` : value passed to `--table`
- `{sourcetable}` : value passed to `--sourcetable`
- `{starttimestamp}` : job start timestamp in `yyyyMMddHHmmss` format
- `{startdate}` : job start date in `yyyyMMdd` format
- `{starttime}` : job start time in `HHmmss` format
- `{starthour}` : job start hour in `HH` format
...
./FastBCP \
  ...
  --sourceschema dbo \
  --sourcetable orders \
  --directory "/data/exports/{database}/{sourceschema}" \

...
##### Tokens available in `--directory` and `--fileoutput`

You can inject placeholders when building the output path and file name. At runtime FastBCP replaces the tokens with the matching execution values so you can automate consistent folder structures and file names.

- `{sourcedatabase}` : value passed to `--sourcedatabase`
- `{sourceschema}` : value passed to `--sourceschema`
- `{sourcetable}` : value passed to `--sourcetable`
- `{starttimestamp}` : job start timestamp in `yyyyMMddHHmmss` format
- `{starttimestamp:yyyyMMddHH}` : job start timestamp formatted in `yyyyMMddHH` format
- `{startdate}` or `{starttimestamp:yyyyMMdd}` : job start date in `yyyyMMdd` format
- `{starttime}` or `{starttimestamp:HHmmss}` : job start time in `HHmmss` format
- `{starthour}` or `{starttimestamp:HH}` : job start hour in `HH` format
- `{user}` : user running the export (from `--user` or the active identity)
- `{filetype}` : output format inferred from the file extension (csv, json, parquet, xlsx, etc.)
- `{datadrivencolumn}` : current value of the data-driven split column when that feature is used
...
./FastBCP \
  ...
  --sourceschema dbo \
  --sourcetable orders \
  --directory "/data/exports/{sourcedatabase}/{sourceschema}" \
  --fileoutput "{sourcetable}_{starttimestamp}.parquet"
```

generates a file named `orders_20241107093000.parquet` (timestamp will vary) inside `/data/exports/tpch10/dbo/`.
...
Specify the parquet compression codec. Possible values are : `None`, `Snappy`, `Gzip`, `Lzo`, `Lz4`, `Zstd` Default is `Zstd` : faster and better compression ratio than others.
